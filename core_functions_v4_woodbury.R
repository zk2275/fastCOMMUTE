...
beta_fastCOMMUTE_generation = function(K, method = "commute",X.tar,X.src,y.tar,y.src,delta,wthreshold) {
  ...
  XTY_tar <- t(X.tar)%*%y.tar
    ...
    if(method == "fastcommute"){
      ...
      for (i in 1:K) {
        GKoff[[i]] <-create.synthetic.ghostknoff.PLANB(scale_src_tar[[i]]*X.tar[1:100,2:201],
                                                               X.tar%*%wthreshold[[i]], # NOT RIGHT
                                                          r=2)
